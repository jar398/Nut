...
c Initialize the variable status table, etc.
      DO 45 I = 1, NVARS
        ...
        SCALE(I) = 1.0
 45   CONTINUE
c Oalpha is a special case.
      STATUS(Voalpha) = FIXED
      NUMINVARS = 0
...
c     *-----------------------------------------------------------------
c     * Ruth's method for calculating root absorbing power (alpha).
c     *-----------------------------------------------------------------
...
c     * function of alpha.
c     * 
c     * N.b. UPALPHA calculates C0 and Pc in addition to oalpha.
c     *-----------------------------------------------------------------
     ...
     >               .TRUE.,
     >               alpha,      ! This parameter appears to be ignored
     >               oalpha, C0, Pc, debugp)
...
c     *-----------------------------------------------------------------
c     * If alpha is calculated, then Pc and C0 are calculated as a side effect.
c     * If alpha is fixed, then we have to compute them.
c     *-----------------------------------------------------------------
      IF (STATUS(Valpha) .NE. CALCULATED .AND.
     >    STATUS(Voalpha) .NE. CALCULATED) THEN
c       -- If alpha is fixed, use it to compute Pc and C0.
        Pc = CALCPc(alpha, gamma, RTRATIO, Deb, EVENSMLR)
        C0 = Pc * Cav
      ENDIF
...
c Root growth can be linear or exponential
...
c     *----------------------------------------------------------*
     ...
     >  CALCPc(myalpha, gamma, RTRATIO, Deb, EVENSMLR)
      IMPLICIT NONE
      DOUBLE PRECISION myalpha, gamma, RTRATIO, Deb, EVENSMLR
      INCLUDE 'Nut.fi'
...
c=======================================================================
c From Carol's file UpAlpha.f

c Old iterative method for computing alpha.
